...
useSeoMeta({
  title: 'Properties for Sale — PropertyHub',
  description: 'Browse all properties available to buy across the US.',
  ogTitle: 'Homes for Sale — PropertyHub',
  ogDescription: 'Find your forever home. Browse premium properties for sale across major US cities.',
...
const { data, pending } = await useAsyncData<PaginatedProperties>(
  'sale-listings',
  () => $fetch('/api/listings', { query: { ...route.query, type: 'sale' } }),
  { watch: [() => route.query] },
)
...
<template>
  <div class="min-h-screen bg-zinc-50 dark:bg-zinc-950">
    <div class="bg-gradient-to-br from-zinc-800 to-zinc-900 text-white py-10">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold mb-1">Properties for Sale</h1>
        <p class="text-zinc-300">Premium listings — find your forever home</p>
      </div>
    </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center gap-3">
        <form class="flex-1 min-w-[180px]" @submit.prevent="submitSearch">
          <div class="relative">
            ...
            <input v-model="searchInput" type="search" placeholder="Search properties for sale…" class="w-full pl-9 pr-4 h-10 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </div>
        </form>
      ...
      <div class="flex gap-8">
        <aside class="hidden lg:block w-72 shrink-0">
          <div class="card p-5 sticky top-36">
            <PropertyFilters ref="filtersRef" locked-type="sale" />
          </div>
        </aside>
        <div class="flex-1 min-w-0">
          <ActiveFilters class="mb-5" locked-type="sale" />
          ...
          <div v-else-if="!data?.items?.length" class="flex flex-col items-center justify-center py-24 text-center card">
            ...
            <NuxtLink to="/listings/sale"><AppButton size="sm" variant="secondary">Clear filters</AppButton></NuxtLink>
          </div>
          <template v-else>
        ...
        <div v-if="mobileFiltersOpen" class="fixed inset-0 z-50 flex">
          ...
          <div class="relative ml-auto w-80 max-w-full h-full bg-white dark:bg-zinc-900 shadow-2xl flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5">
              <PropertyFilters ref="filtersRef" locked-type="sale" @close="mobileFiltersOpen = false" />
            </div>
          </div>
        </div>
      </Transition>
