...
useSeoMeta({
  title: 'All Listings — PropertyHub',
  description: 'Browse all available properties for rent and sale.',
  ogTitle: 'Browse All Properties — PropertyHub',
  ogDescription: 'Filter and search premium properties for rent and sale across the US.',
...
const { data, pending } = await useAsyncData<PaginatedProperties>(
  'listings',
  () => $fetch('/api/listings', { query: route.query }),
  { watch: [() => route.query] },
)
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center gap-3">
        <form class="flex-1 min-w-[180px]" @submit.prevent="submitSearch">
          <div class="relative">
            ...
            <input v-model="searchInput" type="search" placeholder="Search by city or title…" class="w-full pl-9 pr-4 h-10 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </div>
        </form>
        ...
        <button
          class="lg:hidden flex items-center gap-2 h-10 px-4 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm font-medium text-zinc-700 dark:text-zinc-200 transition hover:bg-zinc-50 dark:hover:bg-zinc-750 shrink-0"
        ...
        <p class="text-sm text-zinc-500 dark:text-zinc-400 shrink-0 ml-auto">
          <template v-if="!pending">{{ data?.total ?? 0 }} properties</template>
        </p>
      </div>
    </div>
      ...
      <div class="flex gap-8">
        ...
        <aside class="hidden lg:block w-72 shrink-0">
          <div class="card p-5 sticky top-36">
            <PropertyFilters ref="filtersRef" />
          </div>
        </aside>
        ...
        <div class="flex-1 min-w-0">
          ...
          <div v-else-if="!data?.items?.length" class="flex flex-col items-center justify-center py-24 text-center card">
            <svg xmlns="http://www.w3.org/2000/svg" class="size-12 text-zinc-300 dark:text-zinc-700 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <p class="text-zinc-500 dark:text-zinc-400 font-medium">No properties found</p>
            <p class="text-sm text-zinc-400 dark:text-zinc-500 mt-1 mb-5">Try adjusting your filters or search terms</p>
            <NuxtLink to="/listings"><AppButton size="sm" variant="secondary">Clear all filters</AppButton></NuxtLink>
          </div>
        ...
        <div v-if="mobileFiltersOpen" class="fixed inset-0 z-50 flex">
          ...
          <div class="relative ml-auto w-80 max-w-full h-full bg-white dark:bg-zinc-900 shadow-2xl flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5">
              <PropertyFilters ref="filtersRef" @close="mobileFiltersOpen = false" />
            </div>
          </div>
        </div>
      </Transition>
